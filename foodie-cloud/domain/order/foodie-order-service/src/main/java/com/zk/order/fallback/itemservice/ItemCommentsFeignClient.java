 ...
 * itemCommentsFeignClient继承自ItemCommentsService, 因此相当于在Spring上下文
 ...
 * 具体做法: 可以使用 @EnableFeignClients 注解的clients属性, 只加载需要的Feign接口
 *      优点: 服务提供者和服务调用者都不需要额外的配置
 *      缺点: 启动的时候配置麻烦一点, 需指定加载每一个用到的接口
 *
 ...
 *
 * 3) 原始Feign接口不要定义@FeignClient注解, 这样就不会被加载到上下文当中
 *      优点: 启动的时候直接扫包即可, 不用指定加载接口, 服务提供者不用额外配置
 *      缺点: 任何情况下, 服务调用者都需要声明一个额外的@FeignClient接口
 *
